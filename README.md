...
There is intentionally no goal-directed `deriving` clause. Asking whether a particular proposition
follows is a downstream query over an `Afaik`, not another mode of inspection. Broader proof search
such as Aesop may be added later only when it has a precise inspection-wide role.
...
- a programmatic `Iykyk.wdyk` API with consumer-neutral queries.
...
proof-producing mechanisms remain future work.
